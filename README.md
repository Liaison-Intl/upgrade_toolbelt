...
GKE is providing log facilitie to ease log retrieval and digging.
You can access the 'logging' by navigating to the cloud.google.com as follow :

* Access https://console.cloud.google.com
* Click uper left three horizontal line logo
* Select StackDriver/Logging
* In the first combo box select
  * Container Engine / name of your cluster / 'default' namespaces
* Click to Jump to date box, select as needed
* the 'play' button is also of interest depending of the needs
...
## Docker for the upgrade_toolbelt tool ( legacy )
  ...
  host$ docker run --rm \
    --env 'UTB_OWNER_AND_REPO=benjaminoakes/maid' --env 'UTB_JOB_COUNT=11' \
    -v "$PWD:/opt" \
    -v "$HOME/var/ci":/var/ci \
    -i  \
    -t upgrade_toolbelt \
    sh -l
